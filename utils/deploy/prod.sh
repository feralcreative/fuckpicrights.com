...
# ============================================================================
# fuckpicrights — Production deploy to Synology NAS (Docker-on-NAS, standalone)
# ============================================================================
...
NAS_SSH_PORT="33725"
NAS_DEPLOY_PATH="/volume1/web/fuckpicrights.com"
CONTAINER_NAME="fuckpicrights"
IMAGE_NAME="fuckpicrights:latest"
HOST_PORT="4317"
TARGET_URL="https://fuckpicrights.com"
...
# Dry-run preview
if [ -n "${DRY_RUN:-}" ]; then
  log_info "DRY RUN — would build ${IMAGE_NAME}, transfer to ${NAS_USER}@${NAS_HOST}:${NAS_DEPLOY_PATH}, and (re)start container '${CONTAINER_NAME}' on port ${HOST_PORT}."
  exit 0
fi
...
$SSH_CMD "$SSH_TARGET" "mkdir -p ${NAS_DEPLOY_PATH}/data ${NAS_DEPLOY_PATH}/logs"
